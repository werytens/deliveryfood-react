...
const BasketModal = ({visible, setVisible}) => {
    ...
    if (JSON.parse(localStorage.getItem("deliveryfood")) !== null) {
        ...
        for (const key in db.db) {
            const filteredArray = db.db[key].filter(item => item.hasOwnProperty('id'));
    ...
    const [products, setProducst] = useState([productsCount, userProducts]);
    ...
    const updatePriceRemoveAll = (item) => {
        ...
        let count = 0;
        basket.slice(0, basket.length - 1).split(":").forEach(product => {
            if (product === item.id) {
                count ++
            }
        })
    ...
    return (
        <div className = {
            visible ? [cl.modal, cl.active].join(" ") : cl.modal
            ...
            <div className={cl.modal__inner} onClick={e => e.stopPropagation()}>
                <p className={cl.basket__title}>Корзина</p>
                {
                    products[1].map((item, index) => (
                        <BasketProduct key = {index} props = {[item, products[0][item.id]]} callbacks = {[updatePriceAdd, updatePriceRemove, updatePriceRemoveAll]}/>
                    ))
                }
                <div className={cl.purshate__ending}>
                    <div className={cl.price__all}>
                        {price} ₽
                    </div>
